...
import { saveBase64Image, normalizeImageUrl } from '../helpers.js';
...
inventoryRouter.post('/inventory/batch-upload', authenticateToken, requireOwner, upload.single('file'), async (req: any, res) => {
...
inventoryRouter.post('/inventory/batch-confirm', authenticateToken, requireOwner, express.json({ limit: '50mb' }), (req: any, res) => {
    ...
    for (const sheet of sheetsData) {
      ...
      for (const [rowIdx, row] of sheet.rows.entries()) {
        ...
        const qty = (item.quantity !== undefined && String(item.quantity).trim() !== '')
          ? Number.parseFloat(String(item.quantity)) || 50
          : 50;

        const salePrice = Number.parseFloat(String(item.sale_price  || '')) || null;
        const taxPct    = Number.parseFloat(String(item.tax_percent || '')) || null;
