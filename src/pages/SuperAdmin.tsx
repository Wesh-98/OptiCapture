...
export default function SuperAdmin() {
  ...
  const handleDeleteStore = async () => {
    ...
    setDeleting(true);
    try {
      const res = await fetch(`/api/admin/stores/${deleteStore.id}`, { method: 'DELETE', credentials: 'include' });
      if (res.ok) {
                    ...
                    >
                      ...
                      <td className="px-4 py-4">
                        <div className="flex items-center gap-2">
                          ...
                          <button
                            onClick={() => { setDeleteStore(store); setDeleteConfirmName(''); }}
            ...
            <div className="p-6">
              ...
              <div className="bg-red-50 border border-red-200 rounded-xl p-4 mb-5 text-sm text-red-700 space-y-1">
                ...
                <ul className="list-disc list-inside space-y-0.5 text-red-600 mt-1">
                  <li>All inventory items ({deleteStore.item_count} items)</li>
                  <li>All categories, logs, and scan sessions</li>
                  <li>All user accounts ({deleteStore.user_count} users)</li>
                  <li>The store record itself</li>
                </ul>
              </div>

              <p className="text-sm text-slate-600 mb-2">
                Type <span className="font-mono font-semibold text-slate-800">{deleteStore.name}</span> to confirm:
              </p>
              <input
                type="text"
              ...
              <div className="flex items-center gap-3 justify-end">
                <button
                  onClick={() => { setDeleteStore(null); setDeleteConfirmName(''); }}
                ...
                <button
                  ...
                  disabled={deleteConfirmName !== deleteStore.name || deleting}
                ...
                >
                  {deleting ? <><Loader2 size={14} className="animate-spin" /> Deleting…</> : <><Trash2 size={14} /> Delete Store</>}
                </button>
              </div>
            </div>
          </div>
